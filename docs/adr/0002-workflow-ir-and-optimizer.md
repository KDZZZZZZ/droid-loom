...
- ReAct 是“推理和动作交错”的单步 agent loop 模式，适合表达模型如何在一轮或多轮中思考、调用工具、接收 observation。
...
因此 DroidLoom 的优化最高层不应定在 ReAct loop，而应定在 Agent Graph / ResponseGraph。ReAct 是 graph 节点内部的一种执行模式，不是全局优化的最高 IR。
...
- `ResponseProgram / ResponseIR` 是单个 agent step 的可执行程序对象，包含 input items、instructions、tools、tool visibility、guard binding、verifier binding、cache hints、trace metadata 和 structured output schema。
- `LocalResponseRequest` 是 ResponseProgram 的一次运行实例，是后端调用 ABI。
- ReAct loop 是 `ResponseProgram` 内部可选执行策略，用于 `reason -> act -> observe` 的局部循环。
...
- 工作流持久化后必须维护 IR 版本。
- ReAct-style 自由循环需要被包进有界 `ResponseProgram`，否则无法静态估算成本和风险。
...
- 增加 ReAct-in-ResponseProgram 示例，确保 ReAct 可以作为节点内部策略运行，但不能绕过 graph-level guard/verifier。
