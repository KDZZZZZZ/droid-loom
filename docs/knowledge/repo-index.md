...
| R0-05 | [ggml-org/llama.cpp](https://github.com/ggml-org/llama.cpp) | `llm-backend`, `kv-cache` | 本地 LLM fallback 首选。重点看 Android/NDK 路径、`llama.h` C API、server slots、prompt cache save/restore。DroidLoom 可先用它验证 KV snapshot 和 prompt prefix 复用实验。见 [llama.h 使用文档](./llama-h-usage.md)。 |
