...
DroidLoom 的核心不是“让 LLM 随便点屏幕”，而是把 Android 可观测能力、可执行能力、用户定义 workflow、LLM 推理和编译优化放进同一条受控链路：

```text
screen/API observation -> normalized state -> workflow IR -> planner/agent -> guarded actions -> verifier -> trace
                                      |                         |
                                      +-> compiler passes       +-> local LLM backend
```

推荐路线：

- Android 端用 Kotlin + Jetpack Compose + 前后台服务分层。
- 屏幕语义优先来自 AccessibilityService 的节点树；像素截图只是补充通道。
- 操作优先使用稳定的 Android API、Intent、Shortcut、Notification reply、Accessibility action；手势点击只作为兜底。
- LLM backend 以 MLC LLM 为主，因为它天然使用 TVM/Relax/TIR 风格的编译与打包链路，更适合后续 workflow 编译优化研究；llama.cpp 作为 GGUF 模型生态和快速原型 fallback。
- workflow 先定义独立 IR，再做 prompt/KV/action 层面的编译 pass，不要一开始修改底层推理引擎。
...
- MediaProjection 适合连续屏幕捕获、OCR、视觉定位和不能从 accessibility tree 获得足够语义的场景。
...
- 对 MVP，不建议默认常驻 MediaProjection；应按 workflow 开启，任务结束立即释放。

辅助通道：

- ML Kit Text Recognition 可用于截图 OCR。
- NotificationListenerService 可读通知并执行通知 action，例如回复消息或打开目标 app，但它同样属于敏感能力。
- UI Automator 更适合测试与 benchmark，不适合作为普通用户安装 app 的核心执行通道。
...
不要把 ADB、root、shell 注入作为用户 runtime 前提。它们可用于测试、实验室评估或开发者模式，但不应成为普通 Android app 的基础能力。

## 3. 权限、合规与分发风险

最大风险来自 Accessibility API policy。Google Play 的官方说明中，使用 Accessibility API 进行自动化的 app 必须保证代用户执行的动作具有“狭窄且清晰理解的目的”；使用该 API 让 app 自主发起、规划、执行动作或决定被明确禁止，除非它是符合条件的 accessibility tool。

因此建议：

- 第一阶段定位为 research/prototype/internal build/sideload，不承诺 Google Play 可上架。
- 产品形态上把能力分成“用户定义的确定性 workflow”和“agent 建议 + 用户确认”两类。
...
| Backend | 优点 | 风险 | 结论 |
| --- | --- | --- | --- |
| MLC LLM | Android SDK、模型编译/打包链路、TVM runtime、可配置 `context_window_size` 和 `prefill_chunk_size`，适合研究编译优化 | 构建链复杂，需要物理机 GPU；部分设备/模型布局有已知启动卡顿问题 | 默认主 backend |
| llama.cpp | GGUF 生态成熟，Android 示例和 NDK 交叉编译路径明确，CPU 兼容性好 | 编译优化更偏手写 kernel/backend，workflow 编译与模型编译耦合度较低 | 作为 fallback 和快速原型 |
...
- 0.5B-2B 小模型：分类、slot filling、workflow 分支判定。
...
workflow 应类比 TVM 的计算图，但节点不是 tensor op，而是 agent/runtime op：
...
- Prompt constant folding：系统提示词、工具 schema、workflow 固定说明作为常量段。
...
### 5.3 KV cache lifetime 分析

可先在 workflow 层做 backend-agnostic 的 KV 生命周期规划，而不是先改 MLC/llama.cpp 内核：

- 将 prompt 拆成 `system_prefix`、`workflow_prefix`、`tool_schema`、`goal`、`observation`、`scratchpad`。
- `system_prefix` 与 `workflow_prefix` 在同一模型和 decoding policy 下可跨 workflow session 复用。
...
PagedAttention 的启发是把 KV cache 看成分页内存，从而降低碎片和支持跨请求共享。移动端 MVP 不一定能直接复刻 vLLM 的 serving 模型，但可以借鉴两点：

- prefix block 元数据与引用计数；
- 在 workflow 分支间共享稳定 prefix，屏幕 observation 作为短生命周期 block。
...
```text
app/                  Compose UI, onboarding, permission control
service-accessibility/ AccessibilityService, node tree, actions, screenshots
...
runtime-llm/           MLC and llama.cpp adapters
storage/               Room/DataStore, encrypted trace metadata
benchmark/             UI Automator, AndroidWorld-inspired test harness
```

Android Jetpack：

- Compose：配置、workflow 编辑、trace viewer。
- Room：workflow、trace、model profile、permission grants。
...
- Unit：workflow IR parser、compiler pass、risk policy、prompt segment invalidation。
- JVM/property tests：随机 workflow 图做 dominance/lifetime/kill set 验证。
...
- screenshot/OCR frequency；
- tokens per task；
- prefill latency、decode tok/s、cold start；
- peak RSS/VRAM estimate；
- battery drain；
...
| 风险 | 影响 | 缓解 |
| --- | --- | --- |
| Play policy 不允许自主 accessibility automation | 无法直接上架 | research/internal/sideload 起步；收窄为确定性 workflow；高风险动作用户确认 |
| Accessibility tree 不完整或 app 自定义 canvas | agent 误判 | OCR/MediaProjection 补充；动作前 verifier；局部视觉模型 |
| 本地 LLM 延迟和内存不足 | 体验差 | 小模型、多 profile、context pruning、prefix cache、workflow specialization |
| 手势点击不稳定 | 误操作 | API/action 优先；坐标动作必须由 verifier 包裹 |
| 敏感数据泄露 | 安全事故 | 默认本地、最小日志、截图短生命周期、加密存储、导出前脱敏 |
| workflow 过度动态导致无法优化 | optimizer 价值低 | 限制 IR side effects；schema 化工具；静态 pass + runtime guard 混合 |
